...
function fail(message) {
  console.error('check-site.js : ' + message);
  process.exit(1);
}
...
function lastCommitAgeMs() {
  try {
    var seconds = parseInt(execFileSync('git', ['log', '-1', '--format=%ct'], { cwd: ROOT, encoding: 'utf8' }).trim(), 10);
...
async function main() {
  ...
  await get(adminUrl, 'CMS injoignable');
...
main().catch(function(err) {
  fail('erreur inattendue (' + (err && err.stack ? err.stack : err) + ')');
});
