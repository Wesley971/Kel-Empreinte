...
| `name`, `categories`, `audience` | texte · liste parmi `boucles-oreilles`, `collier`, `noeud-papillon`, `bague`, `bracelet`, `cravate`, `broche` · `femme`, `homme`, `mixte` | obligatoires sauf en brouillon (au moins un type pour publier, jamais de doublon, un brouillon peut n'en avoir aucun) ; les libellés sont dans `catalog.js`. **Une parure ou un combo est une seule pièce à plusieurs types** (KD-120) : un prix, un état, un jeu de photos, rien de vendable séparément. Elle compte sous chacune de ses puces en boutique et répond au filtre dès qu'un de ses types est choisi ; partout (carte, page, meta WhatsApp, « Mes bijoux », `alt` des photos) ses types s'écrivent dans l'ordre du vocabulaire, « Boucles d'oreilles, bracelet, cravate et broche » — jamais un mot « Parure », c'est le nom de sa collection. `mixte` sort sous les deux filtres Femme et Homme. Un type sans pièce n'a pas de puce en boutique |
...
| `createdAt` | date `AAAA-MM-JJ` | obligatoire ; **la date de première mise en vente** — le nom est historique : ses seuls lecteurs (l'ordre de la boutique, plus récentes d'abord dans chaque état ; « Nouveautés » ; le repli de l'accueil ; le `lastmod` du sitemap) veulent le jour où la pièce est devenue visible. Une pièce créée en vente prend le jour de sa création ; un brouillon porte le jour de sa saisie **jusqu'à son passage en vente**, où l'API (fiche ou liste) le remplace par ce jour-là — un brouillon gardé deux mois n'entre pas déjà vieux dans « Nouveautés » ; une pièce retirée puis remise en vente garde sa date (décision du 19/09/2026, KD-93) |
...
Tout champ inconnu, et tout champ retiré du modèle (`heading`, `plainName`, `badge`, `specs`,
`priceType`, `priceConfirmed`, `customizable`, état `bientot` — modèle v1, KD-97 ; `category`, une valeur
devenue la liste `categories`, KD-120), fait échouer le build avec un message explicite qui dit vers quoi
migrer — jamais accepté en silence, ni par l'API ni par le formulaire. Un **brouillon** n'est vérifié que
...
- **Les types suggérés par les collections** (`category` dans `collections.json`, aucun pour Parures : sa
  ...
  coche ses types encore vides sans toucher aux autres cases ; la décocher décoche seulement les types qui
  viennent d'elle et n'ont pas bougé ; dès qu'elle clique une case de type, ce type est le sien (coché ou
  décoché) et plus aucune collection ne le bouge ; une deuxième collection ajoute ses types aux cases vides,
  y compris le vide laissé par une première décochée. Une fiche existante ou dupliquée démarre sans
  suggestion. Une suggestion qui s'annule proprement, jamais un effacement d'un choix à elle — l'origine est
  tracée par la source du geste (`suggestedTypes`, liste de `{ collectionId, category }` dans `piece.js` : le
  code coche sans cliquer, elle ne peut pas choisir sans cliquer).
